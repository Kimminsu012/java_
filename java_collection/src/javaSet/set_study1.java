...
//			System.out.println(name);
		...
		// 차집합
		// 팥을 좋아하는사람 중에 안경을 쓰지않는 사람
		
		HashSet<String> 팥O안씀 = new HashSet<String>();
		it = 팥붕어빵.iterator();
		while(it.hasNext()) {
			String name = it.next();
		...
		// HashSet 메서드를 사용하여 차집합 구하기
		팥O안씀.clear();
		팥O안씀.addAll(팥붕어빵);
		팥O안씀.removeAll(안경쓴사람);
		System.out.println(팥O안씀);
		
		// HashSet 메서드를 사용하여 합집합 구하기
		안경과슈.clear();
		안경과슈.addAll(안경쓴사람);
		안경과슈.addAll(슈붕어빵);
		System.out.println(안경과슈);
		...
		List<String> names = new ArrayList<String>(안경과슈); // HashSet 값을 ArrayList 값으로 바꾸는 과정
		System.out.println( names );
		
	}
	// 교집합 - 두개 이상의 집합에 공통으로 존재하는 값의 집합체 
	// 차집합 - A집합과 B집합에서 A집합에만 존재하는 값
	// 합집합 - A집합과 B집합을 합쳤을 때 중복을 제외 한 값
	

}

// Set - 입력한 데이터 순서 유지 안됨( 힙이라는 메모리 공간을 사용하기 때문에 메모리 주소, 크기 등에 의해 순서가 변경이 된다. )
// 	   - 중복을 허용하지 않는다.
