...
class thread2 implements Runnable { // 인터페이스 사용

	@Override
	public void run() { // Runnable 인터페이스에 있는 메서드
...
class myThread extends Thread{ // 쓰레드 사용
	@Override
	public void run() {
		for(int i=1; i<=10; i++) {
...
// 데몬 스레드 - 주 쓰레드의 작업을 보조 하는 쓰레드
//			   주 쓰레드가 종료 되면 강제적으로 종료된다.
