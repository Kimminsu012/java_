...
public class java_test {
	...
	public static void main(String[] args) {
		...
		for( int i=0; i<list.size(); i++) { // 배열의 크기는 size 메서드로 알 수 있다.
			System.out.println( list.get(i) ); // ArrayList에 저장되어있는 배열의 값을 가져오려면 get메서드를 사용해야 한다.
		}
		
		// 도서 검색 - 검색어와 일치하는 값 찾기 , 검색어가 포함되어있는 값 찾기
		// 책 제목으로 검색 - 검색어와 일치하는 값 찾기
		// 검색 -> 비교(일치하는 값 찾기 - 같다 , 같지않다	 포합 값 찾기 - 범위 검색(같다,같지않다) )
		...
		// 반복문은 for문이 아닌 while문만 가능하다.
		while(it.hasNext()) { // hasNext - 첫 시작부터 하나씩 체크를 하며 진행이 되며 값이 안나오는 구간까지 진행을 한다.
			book data = it.next(); // 다음 공간 이동
