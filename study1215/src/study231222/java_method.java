...
public class java_method {
	...
	public static void main(String[] args) {
		...
		mart_init(); // 제품등록이 첫번재 실행 되어야 한다.
		
		buyer 민수 = buyer_enroll(); // 구매자 등록 (buyer 클래스 객체 생성)
		
		put(민수); // 물건을 구매하기 위한 메서드 , 더이상 구매하지않아야 메서드가 종료된다.
		
		buy(민수);

	}
	static void buy(buyer 민수) {
		while(true) {
			System.out.println( 민수 ); // toString 부분 동작
	...
	static buyer buyer_enroll() {
		...
		buyer 김민수 = new buyer(num,name); 
		김민수.basket = new mart[1]; // 장바구니 공간 1개 생성 - 추후 제품을 장바구니에 담으면서 늘어나게 변경
		return 김민수;
	}
