...
내장객체 - jsp파일이 서버에서 실행 될때 참조 되는 객체들이다.
	     ...
	     pageContext - 현재 jsp ㅔ이지에 실행 가능한 정보를 저장하고있는 객체 , 다른 내장객체 접근 가능
...
<%
	...
	} // 로그인 성공시 user 변수는 값을 가지고, 로그인 하지 않은 상태에서는 user변수는 null을 가진다.
	
	String part = null; // index.jsp에 part 파라미터 값이 들어오면 part파라미터 값을 가지게 되고 part파라미터가 없다면 null을 가진다.
//						   part 마라미터는 페이지 이동시에만 값을 가진다.
