...
// 다형성 ( 상속을 하는 제일 큰 이유 중 하나 )
// - 부모클래스로 자식클래스의 객체를 담을 수 있는 방법
// - 객체의 메서드 통일되어 사용될 수 있다.
// - 하나의 타입으로 여러 타입을 관리 할 수 있기 때문에 유지보수가 좋다.
// - 객체의 재사용성이 쉬워지기 때문에 코드의 재사용성이 높아져 개발자가 편하다.
// - 안정성이 높다.

// 다형성 조건
// 1. 상속관계 : 부모 - 자식 간 클래스 상속
// 2. 오버라이딩 : 메서드의 이름 , 반환타입 , 매개변수가 모두 동일해야한다.
// 				 메서드의 내용은 서로 다르게 작성해도된다.
//               부모 클래스의 메서드를 자식클래스에서도 구현 되어야한다.
//				 자식클래스에 오버라이딩 된 메서드 위에는 @Override 라고 명시해주는게 좋다.
//				 부모클래스로부터 상속받아 재구성된 메서드라고 표기해줘야 다른 개발자도 알아보기 편하고
//				 컴퓨터가 실행하는데 올바른 방식으로 실행 시킬 수 있다.
// 3. 업캐스팅 : 상위 타입으로 타입 변환 ( 자동형 변환 )
...
//		   오버라이딩은 부모클래스에 추상메서드로 구현하고 자식클래스에서 메서드를 구현한다.
