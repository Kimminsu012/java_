...
public class computer {
	...
	class ram{
		...
		public void getRam() {
			System.out.println("apahfl threh : " + ram_speed);
		}
	}
	
	static class vga{ // 내부 static 클래스
		static String brand = "RTX"; // 클래스변수
		int GDDR_SOZE = 8; // 인스턴스변수
		
		public void print() {
//			computer.this.print(); 외부 인스턴스변수는 사용 불가
	...
	public void OS_install() {
		...
		class OS{ // 로컬클래스
			public void getOs() {
//				version =10; // 로컬클래스에서 지역변수는 final이 붙은 상수로 사용된다.
...
// static 클래스 - 내부 static 클래스에는 인스턴스변수, 메서드 와 static변수,메서드 모두 선언이 가능하다.
//				- 외부클래스의 인스턴스변수, 메서드의 사용은 블가능 하다.
//				- 외부 클래스의 객체를 통한 객체 생성이 아니라 독자적으로 생성 가능
//				- static변수, 메서드는 메모리에 하나만 올라가지만
//				  static 클래스는 여러개 생성이 가능하다.
//				- 외부 클래스의 인스턴스를 사용하지 않는 내부클래스라면 내부클래스를 static으로 서언한다. (일부IDE에서는 경고뜬다)
...
//			 - 일회용 클래스
//			 - 익명클래스는 생성자가 없다.
//			 - 익명 클래스는 기존의 클래스 메서드를 재정의 하여 사용 하는 기법이다. 
